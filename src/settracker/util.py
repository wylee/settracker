...
def confirm(message, yes_answers=('y', 'yes')):
    answer = prompt(message, converter=lambda result: result.lower())
    return answer in yes_answers


def prompt(message, converter=None):
    ...
    while not result:
        ...
        result = result.strip()
        if result and converter is not None:
            try:
                result = converter(result)
            except (TypeError, ValueError):
                print(f'Could not convert "{result}" to {converter.__name__}', file=sys.stderr)
                result = None
